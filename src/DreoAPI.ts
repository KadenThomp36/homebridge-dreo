...
export default class DreoAPI {
  ...
  public async authenticate(platform, email, password, server) {
    ...
    await axios
      .post(
        'https://app-api-' + server + '.dreo-cloud.com/api/oauth/login',
        {
  ...
  public async getDevices(platform, auth) {
    ...
    await axios
      .get(
        'https://app-api-' +
          auth.server +
          '.dreo-cloud.com/api/v2/user-device/device/list',
        {
          params: {
            pageSize: 1000,
            currentPage: 1,
  ...
  public async getState(platform, sn, auth) {
    ...
    await axios
      .get(
        'https://app-api-' +
          auth.server +
          '.dreo-cloud.com/api/user-device/device/state',
        {
          params: {
            deviceSn: sn,
            timestamp: Date.now(),
  ...
  public async startWebSocket(platform, auth) {
    ...
    const url =
      'wss://wsb-' +
      auth.server +
      '.dreo-cloud.com/websocket?accessToken=' +
      auth.access_token +
      '&timestamp=' +
      Date.now();
